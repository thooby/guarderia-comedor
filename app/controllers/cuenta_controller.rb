class CuentaController < ApplicationController
  ...
  def look_at_month
    logger.debug "METHOD: look_at_month  -------> #{params}"
    mes = params[:date][:month].to_i
    anno = params[:date][:year].to_i
    @stats = Asiento.informe_mes(mes,anno)
    respond_to do |format|
      format.html # look_at_month.html.erb
      format.json { render :json => @cuenta }
    end
  end
end
